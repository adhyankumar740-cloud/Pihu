...
user_games = {} # Game state is stored here per user ID
...
def db_get_leaderboard(time_filter, scope, chat_id):
    ...
    # Implement actual SQL SELECT/SUM/ORDER BY here.
...
def db_get_all_chat_ids():
    ...
    # Implement actual SQL SELECT here.
    # --- TEMPORARY FALLBACK ---
...
async def process_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if is_game_active and len(text) == 5 and text.isalpha():
        ...
        for guessed_word, emoji_res in game["history"]:
            ...
            blocks_display = " ".join(list(emoji_res)) # Blocks are separated here
        ...
        if text == game["word"]:
            ...
            # Create score entry and add to temporary fallback list
            new_score_entry = {
            ...
            # Ajwa Secret Celebration (DM/Owner only check is already handled below, but added here too for quick feedback)
