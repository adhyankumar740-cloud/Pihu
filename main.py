...
# ⚠️ Ensure this ID is correct for broadcast access
...
# --- Data Storage ---
# Chat histories (for Ajwa) removed
user_games = {} 
...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Sends a message to all recorded chats (Owner only) via reply, preserving the original format exactly (no tags/prefixes)."""
    ...
    content_type = message_to_broadcast.content_type if hasattr(message_to_broadcast, 'content_type') else "UNKNOWN"
    original_text = message_to_broadcast.text
    original_caption = message_to_broadcast.caption
    original_parse_mode = 'Markdown' # Safest assumption for text/captions

    # --- 2. Broadcast Loop (Manual Sending: NO FORWARD TAG) ---
    for chat_id in chat_ids:
        # Avoid sending the broadcast back to the sending chat
        ...
        try:
            ...
            # 9. UNHANDLED TYPES (Poll, Game, Location, Contact, etc.)
            else:
                logger.warning(f"Skipping broadcast for chat {chat_id}: Unsupported content type '{content_type}' for 'as-is' sending. (Not sent to keep 'no tag' rule)")
                continue # Skip this message to maintain the "no tag" rule

            success_count += 1
            await asyncio.sleep(0.1) 
            
        except Exception as e:
            logger.error(f"Manual broadcast failed for chat {chat_id} (Type: {content_type}): {e}")
            
    # --- 3. Final Status ---
    await update.message.reply_text(
        f"✅ **Broadcast Attempt Complete!**\nSent content of type `{content_type.upper()}` to **{success_count}/{total_chats}** chats.\n"
        f"*(Note: Kuch message types jaise ki Polls ya Service Messages skip ho gaye hain taaki 'no tag' rule break na ho.)*"
    )
...
def main() -> None:
    ...
    # Message Handler (Auto-Guess only)
