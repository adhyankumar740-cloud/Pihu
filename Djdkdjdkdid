...
# IMPORTANT: Replace "123456789" with Aadii's actual Telegram User ID
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    # Error Fix: Replaced problematic reply_photo with text reply for safety.
...
async def process_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if is_game_active and len(text) == 5 and text.isalpha():
        ...
        display_message += "\n"
        # Current score calculation: score after this attempt
        score_change = 5 if text == game["word"] else -1
        display_message += f"Attempts: **{game['attempts']}** | Current Score: **{current_score + score_change} pts**\n"
...
def main() -> None:
    ...
    application.add_handler(CommandHandler("getfileid", get_file_id_command)) # NEW COMMAND
